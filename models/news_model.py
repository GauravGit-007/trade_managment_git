...
def save_sentiments_to_db(news_df):
    ...
    conn.commit()
    toolsDB.close_connection(conn)
    print(f"[OK] Inserted {len(news_df)} sentiment records into sentiment_analysis")
...
def run_pipeline():
    ...
    results = sentiment_pipeline(subset, truncation=True, max_length=512)
    ...
    print(f"[OK] News sentiment output saved to: {output_path}")
