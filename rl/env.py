...
class TradingEnv(gym.Env):
    ...
    def step(self, action: int):
        ...
        # --- NEW REWARD LOGIC ---
    # Normalize PnL as return %
        pnl_pct = mtm / prev_price

    # Rolling volatility for risk adjustment (20-bar lookback)
        window = self.df["close"].iloc[max(0, self.current_index-20):self.current_index]
        rolling_vol = window.pct_change().std()
        if np.isnan(rolling_vol) or rolling_vol == 0:
            rolling_vol = 1e-6


        sharpe_like = (pnl_pct - (transaction_cost / prev_price)) / (rolling_vol + 1e-6)


    # Risk penalty scaled by vol
        risk_penalty = self.config.risk_aversion * (abs(self.position) * rolling_vol)

    # Sharpe-like reward
        reward = sharpe_like - risk_penalty
        ...
        info = {
            ...
            "reward": reward,  # log raw reward as well
        }
