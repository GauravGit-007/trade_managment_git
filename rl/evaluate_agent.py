...
Evaluates the accuracy and performance of decisions made by the RL agent.
Now includes an optional hold-aware accuracy metric.
"""
...
def fetch_decisions(conn: sqlite3.Connection, symbol: Optional[str], start: Optional[datetime], end: Optional[datetime]):
    """Fetch decisions for evaluation"""
    cur = conn.cursor()
    where = []
    params = []
    ...
    sql = f"""
        SELECT id, symbol, decision_timestamp, action, position_before, 
               position_after, price, pnl_change, policy_version
        FROM rl_decisions
        {where_sql}
...
def calculate_directional_accuracy_including_holds(decisions: List[Tuple], horizon_hours: int, conn: sqlite3.Connection, hold_threshold: float = 0.0005):
    ...
    action_map = {0: -2, 1: -1, 2: 0, 3: 1, 4: 2}
    for decision in decisions:
        _, symbol, dec_ts_str, action_str, *_ = decision
        try:
            dec_ts = parse_iso_utc(dec_ts_str)
            action = int(action_str)
            executed_delta = float(action_map.get(action, 0))
        ...
        if not end_bar:
            continue
        _, price_end = end_bar
        ret = (float(price_end) - float(price_start)) / float(price_start)
        if executed_delta == 0:
...
def evaluate_agent_performance(symbol: Optional[str] = None, horizon_hours: int = 1, 
    ...
    """Main evaluation function for RL agent performance"""
    ...
    try:
        ...
        print(f"Evaluating {len(decisions)} decisions...")
        ...
        print(f"Total Holds: {total_decisions - total_trades}")
        ...
        hold_threshold = float(os.environ.get("RL_EVAL_HOLD_THRESHOLD", "0.0005"))
        acc_incl_holds = calculate_directional_accuracy_including_holds(decisions, horizon_hours, conn, hold_threshold)
        print(f"Directional Accuracy (Including Holds): {acc_incl_holds:.2f}%")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RL Agent Evaluation Tool",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic evaluation for a symbol
  python rl/evaluate_agent.py --symbol "/ES:XCME{=h}"
  
  # Evaluation with time range
  python rl/evaluate_agent.py --symbol "/ES:XCME{=h}" --start "2024-01-01" --end "2024-01-31"
  
  # Custom evaluation horizon
  python rl/evaluate_agent.py --symbol "/ES:XCME{=h}" --horizon 4
        """
    )
